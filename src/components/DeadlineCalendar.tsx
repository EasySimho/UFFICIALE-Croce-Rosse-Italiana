...
export function DeadlineCalendar({ people }: DeadlineCalendarProps) {
  const getNextDeliveries = (person: Person): Date => {
    ...
    const nextDelivery = new Date(person.deliverySchedule.nextDelivery || person.deliverySchedule.startDate);
      ...
      switch (person.deliverySchedule.type) {
        ...
        case 'custom':
          ...
          if (isBefore(nextDate, currentDate)) {
            return new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, customDays[0]);
          }
          return nextDate;
        default:
          return nextDelivery;
  ...
  const nextDeliveries = people
    .filter(person => !person.completed && person.deliverySchedule?.startDate)
    .map(person => ({
      person,
            ...
            >
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  {person.deliverySchedule.type === 'weekly' && 'Consegna Settimanale'}
                  {person.deliverySchedule.type === 'monthly' && 'Consegna Mensile'}
